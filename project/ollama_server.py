...
from settings import llm_model, num_ctx, embed_model, score_model

def create_llm(num_ctx=num_ctx):
    return OllamaLLM(model=llm_model, base_url="http://localhost:11434", num_ctx=num_ctx)

def create_llm2(num_ctx=num_ctx):
    return OllamaLLM(model=score_model, base_url="http://localhost:11434", num_ctx=num_ctx)
